 ...
 * Provides methods for querying guarantors and their relationships with loans
 * and members.
 */
@Repository
public interface GuarantorRepository extends JpaRepository<Guarantor, Long> {
       ...
       /**
        * Check if an active guarantor relationship exists for a specific loan and
        * member.
        *
        * @param loanId   the loan ID
        * @param memberId the member ID
        * @return true if the member is an active guarantor for the loan, false
        *         otherwise
        */
       @Query("SELECT CASE WHEN COUNT(g) > 0 THEN true ELSE false END FROM Guarantor g " +
        ...
        *
        * @param memberId the member ID
        * @return a list of active guarantors representing loans guaranteed by the
        *         member
        */
       ...
       @Query("SELECT g FROM Guarantor g WHERE g.member.id = :memberId AND g.isActive = :isActive")
       List<Guarantor> findByMemberIdAndIsActive(@Param("memberId") Long memberId, @Param("isActive") boolean isActive);
